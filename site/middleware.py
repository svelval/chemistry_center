...
from site.constants import PATHS_WITHOUT_LOGIN, DEFAULT_LANG_CODE
from quart import redirect, url_for, make_response, abort

from site.cookies import get_or_create_cookie, get_cookie, set_cookie
from site.database_exceptions import ObjectDoesNotExist
...
async def login_required_middleware(request, request_vars):
    if (request.path not in PATHS_WITHOUT_LOGIN) and (not request_vars.path_is_file) and (not request_vars.is_authorized):
        result_response = await make_response(redirect(url_for('game.login', next=request.path)))
